...
private:
  ...
  ControlCommand controlled_stop(const std::string & reason) const;
  // ROS subscriptions update the latest inputs used by the periodic control loop.
  void on_pose(const geometry_msgs::msg::PoseStamped::SharedPtr msg);
  void on_trajectory(const low_speed_av_interfaces::msg::Trajectory::SharedPtr msg);
  void on_vehicle_state(const low_speed_av_interfaces::msg::VehicleState::SharedPtr msg);
  void on_safety_status(const low_speed_av_interfaces::msg::ModuleStatus::SharedPtr msg);
  void on_timer();
  ...
  std::string output_mode_{"scu_control_command"};
  std::string controller_algorithm_{"pure_pursuit"};
  std::string vehicle_model_name_{"front_ackermann"};
  Pose2d pose_;
  VehicleState state_;
  Trajectory trajectory_;
  bool safety_estop_active_{false};
  bool safety_estop_latched_{true};
  uint8_t safety_clear_level_{0};
  std::string safety_clear_state_{"ok"};
  ...
  rclcpp::Subscription<low_speed_av_interfaces::msg::ModuleStatus>::SharedPtr safety_status_sub_;
  rclcpp::Publisher<low_speed_av_interfaces::msg::ControlCommand>::SharedPtr command_pub_;
  rclcpp::Publisher<chassis_interfaces::msg::ScuControlCommand>::SharedPtr scu_command_pub_;
  rclcpp::Publisher<low_speed_av_interfaces::msg::ModuleStatus>::SharedPtr status_pub_;
  rclcpp::Service<low_speed_av_interfaces::srv::SetControllerAlgorithm>::SharedPtr set_algorithm_srv_;
