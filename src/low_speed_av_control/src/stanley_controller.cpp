...
ControlCommand StanleyController::compute(
  const Pose2d & pose,
  ...
  const ControllerOptions & options) const
{
  ...
  const double correction = std::clamp(
    std::atan2(options.stanley_k * lateral, std::abs(state.speed_mps) + options.stanley_epsilon_mps),
  ...
  cmd.desired_curvature_1pm = std::tan(steering) / std::max(options.wheel_base_m, 1e-6);
