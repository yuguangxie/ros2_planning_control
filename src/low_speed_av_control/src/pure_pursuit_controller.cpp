...
ControlCommand PurePursuitController::compute(
  const Pose2d & pose,
  ...
  const ControllerOptions & options) const
{
  ...
  const double lookahead = std::clamp(
    options.lookahead_min_m + std::abs(state.speed_mps) * options.lookahead_speed_gain,
  ...
  const double y_vehicle = -std::sin(pose.yaw_rad) * dx + std::cos(pose.yaw_rad) * dy;
  cmd.desired_curvature_1pm = 2.0 * y_vehicle / std::max(lookahead * lookahead, 1e-6);
